...
def hinter_sampling(min_n_pts, radius=1):
    ...
    :return: Re-ordered 3D points on the sphere surface and a list that indicates on which
    ...
    while len(pts_ordered) != pts.shape[0]:
        # Sort by azimuth  , 排序的key1是方位，key2是海拔
    ...
    pts = pts[np.array(pts_ordered), :] # pts_ordered[0]表示排在现在下标0的是原来的下标为pts_ordered[0]的点
    pts_level = [pts_level[i] for i in pts_ordered]
    pts_order = np.zeros((pts.shape[0],))
    pts_order[np.array(pts_ordered)] = np.arange(pts.shape[0])  # 注意这里是arange，秀
        # pts_order在原始未排序的pts中指出顺序
    for face_id in range(len(faces)):
        faces[face_id] = [pts_order[i] for i in faces[face_id]]  # 点的id更新为排序后的
